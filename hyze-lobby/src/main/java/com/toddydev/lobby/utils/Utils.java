package com.toddydev.lobby.utils;
...
public class Utils {
    ...
    public static Skin getSkin(UUID uniqueId) {
        ...
        String url = "https://sessionserver.mojang.com/session/minecraft/profile/" + uniqueId + "?unsigned=false";
        try {
            JsonObject json = new JsonParser().parse(new InputStreamReader(new URL(url).openStream())).getAsJsonObject();
            String value = json.get("properties").getAsJsonArray().get(0).getAsJsonObject().get("value").getAsString();
            String signature = json.get("properties").getAsJsonArray().get(0).getAsJsonObject().get("signature").getAsString();
    ...
    public static Skin deserializeSkin(String linha) {
        return new Skin(linha.split(",")[0], linha.split(",")[1]);
    }
}
